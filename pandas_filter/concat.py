...
class Concat(object):
    ...
    def __init__(self, workdir_comb, email):
        ...
        # ###########################################
        self.sp_acc_comb = None
        self.single_runs = None  # holds all aln's
        self.sp_counter = {}
    ...
    def run(self, genelistdict, workdir_comb, email, num_threads=None, percentage=0.37, user_concat_fn=None, backbone=False):
        """This is to concatenate different runs into a single alignment and tree.
        ...
        for item in genelistdict.keys():
            ...
            key_list = [item in item_list]
            self.comb_table = pd.concat([self.comb_table, table], keys=key_list)
        ...
        # #####
        self.sp_seq_counter()
        self.get_largest_tre()
        self.make_sp_gene_dict()
        self.make_alns_dict()
        self.concatenate_alns()
        self.get_short_seq_from_concat(percentage)
        self.remove_short_seq()
        self.backbone = backbone
        self.make_concat_table()
        self.write_partition()
        self.write_otu_info()
        self.place_new_seqs(num_threads)

        if backbone is False:
            self.calculate_bootstrap(num_threads)
            self.write_labelled('RAxML_bestTree.autoMRE_fa')
        else:
            self.est_full_tree(num_threads)
    ...
    def combine(self):
        ...
        concat_id_counter = 1
        all_txid = set(self.comb_table['ncbi_txid'])
        for txid in all_txid:
            concat_id_counter += 1
            all_length = []
            for genename in self.comb_table:
                self.genes_present.append(genename)

                entries = self.comb_table[self.comb_table[genename]['ncbi_txid'] == txid]
                all_entries = all_entries.append(entries, key='{}'.format(genename))
                all_length = all_length.append(len(entries))

                rndm_entry = entries.random
                self.comb_table[genename].loc[rndm_entry.index, 'status'] = 500
                self.comb_table[genename].loc[rndm_entry.index, 'status_note'] = 'used for concat'
                concat_id = "concat_{}".format(concat_id_counter)

                self.comb_table[genename].loc[rndm_entry.index, 'concat_id'] = concat_id

                if txid not in self.sp_acc_comb:
                    self.sp_acc_comb = pd.Dataframe(key=txid)
                if genename not in self.sp_acc_comb[txid]:
                    self.sp_acc_comb[txid] = pd.Dataframe(key=genename)

                self.sp_acc_comb[tax_id][genename].append(self.comb_table[genename].loc[rndm_entry.index])
    ...
    def load_single_genes(self, workdir, genename):
        ...
        fn = '{}/papara_alignment.extended'.format(workdir)
        aln = DnaCharacterMatrix.get(path=fn, schema="phylip")
        columns = ['ncbi_txn', 'ncbi_txid', 'org_sp_name', 'tip_name', 'status', "date", 'sseq', 'index']
        table = pd.read_csv('{}/seq_table.csv'.format(workdir), columns=columns)

        self.single_runs[genename] = aln
...
# old stuff:
    ...
    def select_rnd_seq(self, tax_id, gene, del_acc):
        ...
        # self.ld(gene)
        count = 2
        random_gen = random.choice(list(self.comb_table[tax_id][gene]))
        self.comb_table[tax_id][gene][random_gen]["concat:status"] = "used in concat"
        seq = str(self.comb_table[tax_id][gene][random_gen]["seq"])
    ...
    def get_short_seq_from_concat(self, percentage=0.37):
        ...
        """
        print("get_short_seq_from_concat")
        seq_len = {}
        num_tax = 0
        for tax, seq in self.concatenated_aln.items():
                            ...
                            if len(seq_id.split(".")) >= 2:
                                ...
                                if "^ncbi:accession" in val:
                                    ...
                                    if str(seq_id) == str(val['^user:TaxonName']):
                                        tax_id = "taxid_{}".format(
                                            val['^user:TaxonName'].replace("_", "").replace(" ", ""))
                                        found = True
                                        # physcraper.debug("found")
                                    # else:
                            ...
                            if found:
                                ...
                                if tax_id is not None:
                                    ...
                                    for key2, val2 in self.sp_acc_comb[tax_id][gene].items():
                                        ...
                                        if cond:
                                            concat_id = key2
                                            self.sp_acc_comb[tax_id][gene][concat_id][
